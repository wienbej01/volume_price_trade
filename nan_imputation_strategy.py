...
if __name__ == "__main__":
    # Test the imputation strategy
    print("Testing NaN imputation strategy...")
    
    # Create test data with NaN values
    np.random.seed(42)
    data = {
        'open': [100, 101, 102, np.nan, 104, 105, 106, 107, 108, 109],
        'high': [101, 102, 103, np.nan, 105, 106, 107, 108, 109, 110],
        'low': [99, 100, 101, np.nan, 103, 104, 105, 106, 107, 108],
        'close': [100.5, 101.5, 102.5, np.nan, 104.5, 105.5, 106.5, 107.5, 108.5, 109.5],
        'volume': [1000, 1100, 1200, np.nan, 1400, 1500, 1600, 1700, 1800, 1900],
        'atr_20': [1.0, 1.1, 1.2, np.nan, np.nan, 1.5, 1.6, 1.7, 1.8, 1.9],
        'vp_dist_to_poc_atr': [0.5, 0.6, 0.7, np.nan, np.nan, np.nan, 1.1, 1.2, 1.3, 1.4],
        'vpa_climax_up': [0, 1, 0, np.nan, 0, 1, 0, 0, 1, 0],
    }
    ...
    print(df)
    print(f"\nOriginal NaN count: {df.isna().sum().sum()}")
    
    # Apply imputation
    custom_config = create_custom_imputation_config()
    imputed_df = apply_nan_imputation(df, custom_methods=custom_config)
    
    print("\nImputed DataFrame:")
    print(imputed_df)
    print(f"\nImputed NaN count: {imputed_df.isna().sum().sum()}")