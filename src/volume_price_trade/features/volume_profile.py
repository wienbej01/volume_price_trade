...
def compute_volume_profile_features(df: pd.DataFrame, cfg: Dict[str, Any]) -> pd.DataFrame:
    ...
    for i, date in enumerate(unique_dates):
        ...
        start_idx = max(0, i - rolling_sessions + 1)
        ...
        val, vah = calculate_value_area(profile, value_area_pct)
        
        # Find HVN/LVN
        hvn_prices, lvn_prices = find_hvn_lvn(profile, hvn_lvn_threshold)
        
        # Update features for each bar in current session
        for idx, row in current_session.iterrows():
            # Get ATR value for this bar
            current_atr = atr_values.loc[idx]
        ...
        if i > 0:
            ...
            if not prev_session.empty:
                ...
                prev_profile = calculate_volume_profile(prev_session, bin_size)
                
                if not prev_profile.empty:
                    prev_poc = find_poc(prev_profile)
